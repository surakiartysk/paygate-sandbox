...
import { getPayment, appendCallbackHistory } from './storage.js';
...
export async function sendCallback(invoiceNo, options = {}) {
  ...
  // Appended through storage rather than composed here on purpose. `payment`
  // was read before `executeCallback` went out over the network, so building
  // the new history from it would append to a snapshot that is one full HTTP
  // round trip old — and two concurrent callbacks on one invoice would each
  // overwrite the other's entry while both were answered with success.
...
export async function executeCallback(url, payload, paymentOrInvoiceNo = null) {
  ...
  try {
    ...
    // which is only true because the fetch below does not follow redirects.
    // See the note on `redirect` there.
    ...
    const res = await fetch(url, {
      ...
      // Redirects are not followed, and this is a security boundary rather
      // than a preference.
      //
      // `fetch` defaults to `redirect: 'follow'`, which put every outbound
      // callback one Location header away from anywhere. The guard above runs
      // on the URL it was given; the runtime then followed whatever came back,
      // without asking again. A caller supplies a public backendReturnUrl that
      // passes the guard, their server answers 302 pointing at a private
      // address, and the request is made — the choke point this comment claims
      // to be, routed around by a header. Verified against
      // http://169.254.169.254/ before this was set.
      //
      // Following safely would mean re-validating each hop and capping the
      // chain. Not following is simpler and loses nothing worth having: a
      // 3xx is reported as the result, so an integration whose endpoint
      // redirects can see that and point the sandbox at the real one.
      redirect: 'manual'
    });
...
export async function sendCallbackSequence(invoiceNo, sequence, options = {}) {
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    // Re-reading here was what kept sequences correct while single callbacks
    // lost entries; this is the same fix moved into storage, where the local
    // path can do it without an await in the middle at all.
    await appendCallbackHistory(invoiceNo, historyEntry);
