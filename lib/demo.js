...
export async function seedDemoPayments({ force = false } = {}) {
  ...
  for (const [index, template] of DEMO_TEMPLATES.entries()) {
    ...
    const payment = {
      ...
      isDemo: true,
      statusHistory: [
        { status: 'pending', timestamp: createdAt, note: 'Payment created (demo data)' }
      ]
    };
    ...
    if (isSettled) {
      ...
      payment.statusHistory.push({
        status: 'success',
        timestamp: createdAt,
        note: 'Payment completed (demo data)'
      });
    }
...
export async function runDemoScenario(request) {
  ...
  await savePayment({
    ...
    callbackCount: 0,
    isDemo: true,
    statusHistory: [{ status: 'pending', timestamp: createdAt, note: 'Payment created (guided demo)' }]
  });
